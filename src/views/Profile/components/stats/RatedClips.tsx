...
const RatedClips: React.FC<RatedClipsProps> = ({ ratingsData, clipsData, location }) => {
    ...
    const getRatingColor = (rating: string | number) => {
        if (rating === 1 || rating === '1') return "bg-[#f23030]";
            ...
            {isLoading ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 p-4">
                    ...
                    <motion.div
                        ...
                        className="flex flex-col items-center justify-center py-16 text-center bg-[#0e1315] rounded-[10px] border border-[#263238]"
                    >
                        <div className="w-20 h-20 bg-[#161d21] rounded-[10px] flex items-center justify-center mb-6 border border-[#263238]">
                            <div className="text-4xl">🎬</div>
                        </div>
                        <h3 className="text-2xl font-bold text-[#e6e6e6] mb-3">
                            No rated clips found
                        </h3>
                        <p className="text-[#b3b3b3] max-w-md mb-8 leading-relaxed">
                            You haven't rated any clips yet. Visit the clip browser to start rating clips and help the team!
                        </p>
                        <Link
                            to="/clips"
                            className="bg-[#f23030] hover:bg-[#d92626] text-white px-8 py-3 rounded-[10px] font-semibold transition-all duration-200"
                        >
                            Browse Clips
                        </Link>
                    </motion.div>
                ...
                <>
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 p-4">
                        {currentClips.map((clip: ExtendedClip) => (
                            <motion.div
                                ...
                                className="relative group aspect-video rounded-[10px] overflow-hidden hover:shadow-xl transition-all duration-300 border border-[#263238]"
                            >
                                <Link
                                    to={`/clips/${clip._id}`}
                                    state={{ from: location }}
                    ...
                    {totalPages > 1 && (
                        <div className="flex justify-center mt-8 mb-4">
                            <div className="flex items-center space-x-2 bg-[#161d21] rounded-[10px] p-2 border border-[#263238]">
                                <motion.button
                                    whileHover={{ scale: 1.05 }}
                                    whileTap={{ scale: 0.95 }}
                                    onClick={() => paginate(Math.max(1, currentPage - 1))}
                                    disabled={currentPage === 1}
                                    className={`px-4 py-2 rounded-[8px] font-medium transition-all duration-200 ${currentPage === 1
                                        ? 'bg-[#0e1315] text-[#626262] cursor-not-allowed'
                                        : 'bg-[#0e1315] hover:bg-[#263238] text-[#b3b3b3]'
                                        }`}
                                >
                                    Previous
                                </motion.button>
                                ...
                                <div className="flex flex-wrap justify-center gap-1 max-w-lg mx-2">
                                    {[...Array(totalPages).keys()].map(number => (
                                        <motion.button
                                            ...
                                            className={`w-10 h-10 rounded-[8px] flex items-center justify-center font-semibold transition-all duration-200 ${currentPage === number + 1
                                ...
                                <motion.button
                                    ...
                                    className={`px-4 py-2 rounded-[8px] font-medium transition-all duration-200 ${currentPage === totalPages
                                        ? 'bg-[#0e1315] text-[#626262] cursor-not-allowed'
                                        : 'bg-[#0e1315] hover:bg-[#263238] text-[#b3b3b3]'
                                        }`}
                                >
