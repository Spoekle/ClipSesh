...
/**
 * Clear all read notifications (or all notifications if all=true)
 */
export const clearNotifications = async (all = false): Promise<void> => {
  try {
    await axios.delete(
      `${backendUrl}/api/notifications${all ? '?all=true' : ''}`,
      { headers: getAuthHeaders() }
    );
  } catch (error) {
