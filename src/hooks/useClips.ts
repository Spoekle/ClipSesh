...
// Hook for getting adjacent clips from cached data with API fallback
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (!cachedData) {
        ...
        const clipsQueries: any[] = allCachedQueries.filter(query =>
        ...
        for (const query of clipsQueries) {
          ...
          // Score based on matching parameters - prioritize exact matches
          // Sort-related parameters (high priority)
          if (queryParams.sortOption === (params as any).sortOption) score += 15;
          if (queryParams.sortBy === (params as any).sortBy) score += 10;
          if (queryParams.sortOrder === (params as any).sortOrder) score += 5;
          if (queryParams.sort === (params as any).sort) score += 5;

          // Filter parameters (medium priority)
          if (queryParams.streamer === (params as any).streamer) score += 8;
          if (queryParams.excludeRatedByUser === (params as any).excludeRatedByUser) score += 8;
          if (queryParams.excludeDeniedClips === (params as any).excludeDeniedClips) score += 5;
          if (queryParams.search === (params as any).search) score += 5;

          // Other parameters (lower priority)
      ...
      if (!cachedData) {
        console.log('❌ No cached clips data found, trying API fallback...');
        // API fallback - fetch adjacent clips from the server
        try {
          const sortOption = (params as any).sortOption || 'newest';
          const apiResult = await clipService.getAdjacentClips(clipId, {
            sort: sortOption,
            streamer: (params as any).streamer,
            excludeRatedByUser: (params as any).excludeRatedByUser,
          });
          console.log('✅ API fallback returned:', { prev: apiResult.previous?._id, next: apiResult.next?._id });
          return { previous: apiResult.previous || null, next: apiResult.next || null };
        } catch (apiError) {
      ...
      if (allClips.length === 0) {
        console.log('❌ No clips found in cached data, trying API fallback...');
        // API fallback
        try {
          const sortOption = (params as any).sortOption || 'newest';
          const apiResult = await clipService.getAdjacentClips(clipId, {
      ...
      if (currentIndex === -1) {
        console.log(`❌ Current clip ${clipId} not found in cached data, trying API fallback...`);

        // API fallback when clip not in cache
        try {
          const sortOption = (params as any).sortOption || 'newest';
          const apiResult = await clipService.getAdjacentClips(clipId, {
            sort: sortOption,
